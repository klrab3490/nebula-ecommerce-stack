    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all" className="space-y-4">
        <TabsList className="grid w-full grid-cols-6">
          <TabsTrigger value="all" className="text-xs">All ({ordersByStatus.all})</TabsTrigger>
          <TabsTrigger value="pending" className="text-xs">Pending ({ordersByStatus.pending})</TabsTrigger>
          <TabsTrigger value="processing" className="text-xs">Processing ({ordersByStatus.processing})</TabsTrigger>
          <TabsTrigger value="shipped" className="text-xs">Shipped ({ordersByStatus.shipped})</TabsTrigger>
          <TabsTrigger value="delivered" className="text-xs">Delivered ({ordersByStatus.delivered})</TabsTrigger>
          <TabsTrigger value="cancelled" className="text-xs">Cancelled ({ordersByStatus.cancelled})</TabsTrigger>
        </TabsList>
            ...
            <CardContent>
              {filteredOrders.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">{searchTerm ? 'Try adjusting your search' : 'Orders will appear here as customers place them'}</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                      ...
                      <TableRow key={order.id}>
                        <TableCell className="font-medium">
                          <div>
                            ...
                            {order.trackingNumber && (
                              <div className="text-xs text-muted-foreground">Track: {order.trackingNumber}</div>
                            )}
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">{order.items[0]?.name}{order.items.length > 1 && ` +${order.items.length - 1} more`}</div>
                          </div>
                        </TableCell>
        ...
        {(['pending', 'processing', 'shipped', 'delivered', 'cancelled'] as const).map(status => (
          <TabsContent key={status} value={status}>
            <Card>
              <CardHeader>
                <CardTitle>{status.charAt(0).toUpperCase() + status.slice(1)} Orders ({orders.filter(o => o.status === status).length})</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-muted-foreground">Orders with {status} status would be displayed here with the same table structure.</p>
              </CardContent>
            </Card>
          </TabsContent>
        ))}
