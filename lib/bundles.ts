...
function calculateSingleBundleDiscount(
    ...
    for (const bundleProduct of requiredProducts) {
        ...
        originalPrice +=
            (bundleProduct.product.discountedPrice || bundleProduct.product.price) * usedQuantity;
    ...
    switch (bundle.discountType) {
        ...
        case "buy_x_get_y":
            ...
            const freeItems =
                Math.floor(totalQuantityInBundle / bundle.minQuantity) * bundle.discountValue;
...
export function applyBestBundleDiscounts(
    ...
    for (const discount of availableDiscounts) {
        const hasOverlap = discount.appliedProducts.some((productId) =>
