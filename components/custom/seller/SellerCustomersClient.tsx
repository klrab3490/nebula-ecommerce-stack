        ...
        <CardContent className="p-4">
          <div className="flex flex-col gap-4 md:flex-row">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by name, email, username, or UUID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
        ...
        <CardContent>
          {filteredUsers.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">{searchTerm || roleFilter !== 'all' || statusFilter !== 'all' ? 'Try adjusting your search or filters' : 'Users will appear here as they join the platform'}</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.uuid}>
                    ...
                    <TableCell>
                      <div className="font-mono text-xs">{user.uuid.slice(0, 8)}...{user.uuid.slice(-8)}</div>
                    </TableCell>
                    <TableCell>
                      <div>
                    ...
                    <TableCell className="text-sm text-muted-foreground">{new Date(user.joinDate).toLocaleDateString()}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{user.lastLogin ? new Date(user.lastLogin).toLocaleDateString() : 'Never'}</TableCell>
                    <TableCell>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {user.status === 'suspended' ? (
                            <DropdownMenuItem><Check className="h-4 w-4 mr-2" />Unsuspend</DropdownMenuItem>
                          ) : (
                            <DropdownMenuItem className="text-red-600"><Ban className="h-4 w-4 mr-2" />Suspend User</DropdownMenuItem>
                          )}
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <Card className="max-w-2xl w-full max-h-[90vh] overflow-auto">
            ...
            <CardContent className="space-y-6">
              <div>
                <h3 className="font-semibold mb-3 flex items-center gap-2"><User className="h-4 w-4" />Basic Information</h3>
              ...
              <div>
                <h3 className="font-semibold mb-3 flex items-center gap-2"><Mail className="h-4 w-4" />Contact Information</h3>
              ...
              <div>
                <h3 className="font-semibold mb-3 flex items-center gap-2"><Activity className="h-4 w-4" />Activity & Statistics</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
              ...
              <div className="flex gap-2 pt-4 border-t">
                <Button variant="outline" className="flex-1"><Edit className="h-4 w-4 mr-2" />Edit User</Button>
                {selectedUser.status === 'suspended' ? (
                  <Button variant="default" className="flex-1"><Check className="h-4 w-4 mr-2" />Unsuspend User</Button>
                ) : (
                  <Button variant="destructive" className="flex-1"><Ban className="h-4 w-4 mr-2" />Suspend User</Button>
                )}
              </div>
            </CardContent>
          </Card>
